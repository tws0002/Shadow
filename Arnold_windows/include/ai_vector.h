 ...
 */
inline AtPoint2 AiPoint2(float x, float y)
{
   AtPoint2 out = {x, y};
   return out;
}
...
/**
 * Create a XY vector from its components (same as AiPoint2)
 */
inline AtVector2 AiVector2(float x, float y)
{
   AtVector2 out = {x, y};
   return out;
}
 ...
 */
inline AtPoint AiPoint(float x, float y, float z)
{
   AtPoint out = {x, y, z};
   return out;
}
...
/**
 * Create a XYZ vector from its components (same as AiPoint)
 */
inline AtVector AiVector(float x, float y, float z)
{
   AtVector out = {x, y, z};
   return out;
}
 ...
 */
AI_API AI_PURE bool AiV3Exists(const AtVector& a);
 ...
 *
 * This does not actually check for zero, it checks for small values, including
 * zero.  This can easily be confused with asking for exact comparisons with zero,
 * so this confusing function is deprecated and should be avoided
 */
AI_DEPRECATED
...
AI_DEPRECATED
inline bool AiV3isZero(const AtVector& a, float epsilon = AI_EPSILON)
{
   return AiV3IsSmall(a, epsilon);
}
...
inline void AiV2Create(AtVector2& out, float x, float y)
{
   out.x = x;
   out.y = y;
}
