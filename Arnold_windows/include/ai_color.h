 ...
 */
AI_API AI_PURE bool AiRGBCorrupted(const AtRGB& rgba);

/**
 * Check to see if an RGBA color has any corrupted components (nan or infinite).
 */
AI_API AI_PURE bool AiRGBACorrupted(const AtRGBA& rgba);
...
AI_API AI_PURE AtColor AiColorHeatMap(const AtColor* map_colors, const float* map_values, unsigned int map_length, float lookup);
